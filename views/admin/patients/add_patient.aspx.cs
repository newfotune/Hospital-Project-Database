...
public partial class views_Admin_Patients_admin_patient_add : System.Web.UI.Page
{
    ...
    protected void add_click(object sender, EventArgs e)
    {
        ...
        float _weight = float.Parse(weight.Value);
        float _height = float.Parse(height.Value);

        error.InnerHtml = "";
        CheckSuccess(DBManager.AddPerson(_fname, _lname, _gender, 2, _DOB, _phone, _email, _address), "Person");
        // DBManager.fixAutoIncreament();
        CheckSuccess(DBManager.AddPatient(_height, _weight), "Patient");
